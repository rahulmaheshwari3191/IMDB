...
class TestIMDbMovieRanking(unittest.TestCase):
    ...
    def test_ranking_logic(self):
        ...
        # Get the top 10 movies with ranking scores
        ...
        top_movies.select("primaryTitle").show()

        # Finding top 10 most credited person for above top 10 movies
        top_movie_ids = [row["tconst"] for row in top_movies.collect()]

        # Get credited persons for the top movies
        top_movies_credits = self.principals_df.filter(col("tconst").isin(top_movie_ids))

        # Group by credited person (nconst) and count their appearances in the top movies
        credited_persons = top_movies_credits.groupBy("nconst").agg(count("tconst").alias("count")) \
        .orderBy(desc("count")).limit(10)

        # Show most credited persons for top 10 movies
        print("Most Credited Persons for Top Movies:")
